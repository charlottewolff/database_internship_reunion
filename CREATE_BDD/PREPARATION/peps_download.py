...
if len(sys.argv) == 1:
    ...
    parser.add_option("-t", "--tile",dest="tile", action="store", type="string", \
            help="Tile numbers (ex: T31TCJ,T30TCJ), Sentinel2 only",default=None)
...
if options.collection=='S2':
    ...
    if options.tile!=None:
        print "**** Tile option can not be used with products before '2016-12-05'"
...
for i in range(len(data["features"])):    
    ...
    test = str(data["features"][i]["properties"]["productIdentifier"])
    ...
    if options.tile == None:
        ...
        for tile in location:
            if tile in test:
                print("Tile in the selection ---> Download")
                if options.orbit!=None:
                    if platform.startswith('S2'):
                        if prod.find("_R%03d"%options.orbit)>0:
                            download_dict[prod]=feature_id
...
if len(download_dict)==0:
    ...
    folder = open('number.txt', 'w')
    folder.write(str(len(download_dict)))
    folder.close()
else:
    print "********************** Number of images to download : ",len(download_dict)
    folder = open('number.txt', 'w')
    folder.write(str(len(download_dict)))
    folder.close()
